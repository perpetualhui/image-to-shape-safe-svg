...
- 默认输出独立 SVG 文件；ZIP 和多页验收报告都需要用户明确要求或确认。
- 使用“一段语义段落一个可编辑文本框”，并统一采用 Microsoft YaHei 字体和固定字号体系。
...
默认输出是独立 SVG 文件。如果需要目录结构，使用：

```text
svg_shape_safe/
```

不要默认创建 ZIP，也不要默认生成多页验收报告，除非用户明确要求或确认。
...
1. 检查源图尺寸和页面结构。
2. 先写简短 preflight：复杂度、图标数量、文字密度风险、布局策略、视觉模型调用预算、icon 保留决策状态、字体字号假设和多页 report 偏好。
3. 转换前必须询问用户是否要保留所有源图标，还是允许简化装饰性/重复图标。
4. 多页转换时，生成验收报告前必须询问用户是否需要。
5. 用可编辑 SVG primitives 重建页面。
6. 一段语义段落必须是一个可编辑文本框。
7. 全页/全批统一使用固定字号体系：`24 / 18 / 16 / 14 / 12 / 10 / 8`。
8. 控制图标预算，只保留真正有信息价值的图标。
9. 箭头用 line/path 加 polygon 箭头头部绘制。
10. 用内置工具验证 SVG。
11. 渲染预览并检查错位、遮挡、箭头缺失和 Office 转换风险。
